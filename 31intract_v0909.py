...
def My_debug(p_path):
    ...
    try:
        # 读取 Shapefile 文件
        KM_ply,file_encoding = read_file_with_encoding(p_path)

        KM_ply = del_old_filed(KM_ply)

        print("准备保存到原文件路径")
        # # 构造新的文件路径（原文件名 + _temp）
        # file_directory, file_name = os.path.split(p_path)
        # base_name, ext = os.path.splitext(file_name)
        # new_file_name = f"待赋值0911B{ext}"
        # new_file_path = os.path.join(file_directory, new_file_name)
        KM_ply.to_file(p_path, encoding=file_encoding)
        tkinter.messagebox.showinfo("完成", f"处理完成！结果已保存到{p_path}。")
...
def do_work(p_shp_path, p_aux_path, p_output_path):
    try:
        ...
        # 确保所有文件使用相同的坐标参考系
        if KSJC_ply.crs != KQ_ply.crs:
            KQ_ply = KQ_ply.to_crs(KSJC_ply.crs)
        if KSJC_ply.crs != XZQ_ply.crs:
            XZQ_ply = XZQ_ply.to_crs(KSJC_ply.crs)
        if KSJC_ply.crs != BHQ_ply.crs:
            BHQ_ply = BHQ_ply.to_crs(KSJC_ply.crs)
        if KSJC_ply.crs != ZDQ_ply.crs:
            ZDQ_ply = ZDQ_ply.to_crs(KSJC_ply.crs)
        if KSJC_ply.crs != rail_buf_ply.crs:
            rail_buf_ply = rail_buf_ply.to_crs(KSJC_ply.crs)
        ...
        # 将几何转换到投影坐标系
        projected_ply = KSJC_ply.to_crs(epsg=4508)  # 使用合适的高斯投影区域 CGS2000的111°中央经线 epsg为4508
        ...
        # 保留F_ID列
        # KSJC_ply = KSJC_ply.reset_index().set_index('F_ID_copy').rename_axis('F_ID')
